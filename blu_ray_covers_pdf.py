...
if remove_index is not None:
    ...
    removed_name = list(st.session_state.quantities.keys())[remove_index]
    st.session_state.quantities.pop(removed_name, None)

# Funkcja tworzenia PDF
def generate_pdf(cover_data):
    ...
    positions = [
        (15 * mm, page_size[1] - horizontal_size[1] - 15 * mm),   # górna
        (15 * mm, page_size[1] - 2 * horizontal_size[1] - 20 * mm),  # dolna
        (horizontal_size[0] + 20 * mm, (page_size[1] - vertical_size[1]) / 2)  # pionowa
    ]
            ...
            if len(current_batch) == 3:
                for idx, img in enumerate(current_batch):
                    if idx == 2:
                        ...
                        c.drawImage(img_reader, positions[idx][0], positions[idx][1],
                                    vertical_size[0], vertical_size[1])
                    else:
                        img_reader = ImageReader(img)
                        c.drawImage(img_reader, positions[idx][0], positions[idx][1],
                                    horizontal_size[0], horizontal_size[1])
                c.showPage()
                current_batch = []
